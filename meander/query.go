...
// APIKey はhttps://maps.googleapis.com/maps/api/placeのAPI Key
var APIKey string
...
// Place はGoogle Places API(https://developers.google.com/places/web-service/search)におけるPlaceを表す
type Place struct {
...
// Query はGoogle Places APIへのリクエスト用クエリを表す
type Query struct {
	Lat          float64
	Lng          float64
	Journey      []string
